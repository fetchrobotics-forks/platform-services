...
go build -asmflags -trimpath -ldflags "-X github.com/fetchcore-forks/platform-services/internal/version.GitHash=$HASH" -o cmd/echosrv/bin/echosrv cmd/echosrv/*.go
go build -asmflags -trimpath -ldflags "-X github.com/fetchcore-forks/platform-services/internal/version.GitHash=$HASH" -race -o cmd/echosrv/bin/echosrv-race cmd/echosrv/*.go
if ! [ -z "${TRAVIS_TAG}" ]; then
    if ! [ -z "${GITHUB_TOKEN}" ]; then
        gothub-release release --user karlmutch --repo platform-services --tag ${TRAVIS_TAG} --pre-release || true
        gothub-release upload --user karlmutch --repo platform-services  --tag ${TRAVIS_TAG} --name echosrv --file cmd/echosrv/bin/echosrv
    fi
fi
