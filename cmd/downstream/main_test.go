...
//
// cd $(GOROOT)/src/github.com/fetchcore-forks/platform-services/cmd/downstream
...
// server binary for the go downstream server using the command below.  The difference being that the
...
//
// ./bin/downstream-cpu-run-coverage -test.run "^TestRunMain$" -test.coverprofile=system.out
//
