...
go build -asmflags -trimpath -ldflags "-X github.com/fetchcore-forks/platform-services/internal/version.GitHash=$HASH" -o cmd/timesrv/bin/timesrv cmd/timesrv/*.go
go build -asmflags -trimpath -ldflags "-X github.com/fetchcore-forks/platform-services/internal/version.GitHash=$HASH" -race -o cmd/timesrv/bin/timesrv-race cmd/timesrv/*.go
if ! [ -z "${TRAVIS_TAG}" ]; then
    if ! [ -z "${GITHUB_TOKEN}" ]; then
        gothub-release release --user karlmutch --repo platform-services --tag ${TRAVIS_TAG} --pre-release || true
        gothub-release upload --user karlmutch --repo platform-services  --tag ${TRAVIS_TAG} --name timesrv --file cmd/timesrv/bin/timesrv
    fi
fi
